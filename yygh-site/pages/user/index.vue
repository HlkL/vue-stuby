...
export default {
  ...
  methods: {
    ...
    getDict() {
      dictApi.queryChlidNodeByDictCode("CertificatesType").then((response) => {
        this.certificatesTypeList = response.data;
      });
    },
    onUploadSuccess(response, file) {
    /**
     * 证件图不显示，可以更改onUploadSuccess里的限定条件， 改为 response === null，
     *  然后this.userAuth.certificatesUrl = response;
     */
      if (response === null) {
        // this.$message.error("上传失败");
        this.userAuth.certificatesUrl = response;
        return;
      }
      // 填充上传文件列表
