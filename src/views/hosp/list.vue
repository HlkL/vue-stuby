<template>
    ...
    <div class="app-container">
        ...
        <el-table v-loading="listLoading" :data="list" border fit highlight-current-row>
            ...
            <el-table-column prop="param.addressString" label="详情地址"/>
            ...
            <el-table-column label="操作" width="230" align="center">
                ...
                <template slot-scope="scope">
                    <el-button v-if="scope.row.status == 1"  type="danger" plain=true size="mini" @click="updateStatus(scope.row.id, 0)">下线</el-button>
                    <el-button v-if="scope.row.status == 0"  type="success" plain=true size="mini" @click="updateStatus(scope.row.id, 1)">上线</el-button>

                    <router-link :to="'/hospSet/hosp/show/'+scope.row.id">
                        <el-button type="warning" plain=true size="mini">查看</el-button>
                    </router-link>

                    <router-link :to="'/hospSet/department/schedule/'+scope.row.hoscode">
                        <el-button type="primary" plain=true size="mini">排班</el-button>
                    </router-link>
...
export default {
    ...
    methods: {
        ...
        updateStatus( id,status ){
            hospitalApi.updateHospStatus( id,status ).then( response =>{
                this.$message({
                    type: 'success',
                    message: '设置成功!'
                })
